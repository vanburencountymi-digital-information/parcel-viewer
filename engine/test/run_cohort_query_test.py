...
class CohortQueryTest(unittest.TestCase):
    ...
    def test_explicit_pins_build_parcel_no_predicate(self):
        pred, params, resolved = cq.build_predicate(
            {"type": "explicit", "pins": ["80-07-017-025-00"]}, 3000
        )
    ...
    def test_explicit_ids_and_pins_are_ORed(self):
        pred, params, resolved = cq.build_predicate(
            {"type": "explicit", "ids": [3], "pins": ["A", "B"]}, 3000
        )
    ...
    def test_buffer_requires_positive_distance(self):
        for bad in (
            {"type": "buffer", "parcel_id": 1, "distance_ft": 0},
    ...
    def test_named_geography_unknown_or_empty_fails_closed(self):
        for bad in (
            {"type": "named-geography", "geography": "county", "name": "x"},
    ...
    def test_drawn_polygon_intersects_transformed_geojson(self):
        poly = {
            "type": "Polygon",
            "coordinates": [[[-86.0, 42.2], [-86.0, 42.3], [-85.9, 42.3], [-86.0, 42.2]]],
        }
        pred, params, resolved = cq.build_predicate(
            {"type": "drawn-polygon", "geometry": poly}, 3000
        )
    ...
    def test_drawn_polygon_rejects_non_polygon_and_empty(self):
        for bad in (
            {"type": "drawn-polygon", "geometry": {"type": "Point", "coordinates": [1, 2]}},
    ...
    def test_drawn_polygon_rejects_excessive_vertices(self):
        ...
        with self.assertRaises(cq.CohortSelectorError):
            cq.build_predicate(
                {"type": "drawn-polygon", "geometry": {"type": "Polygon", "coordinates": [ring]}},
                3000,
            )
    ...
    def test_geography_sources_are_whitelisted_and_consistent(self):
        ...
        for src in cq.GEOGRAPHY_SOURCES.values():
            self.assertIn(src["kind"], ("spatial", "attribute"))
            if src["kind"] == "spatial":
                self.assertTrue(src["table"].startswith("geo."))
                self.assertIn("id_col", src)
                self.assertIn("name_col", src)
            else:
    ...
    def test_malformed_values_raise_selector_error_not_500(self):
        ...
        for sel in bad:
            with self.subTest(sel=sel), self.assertRaises(cq.CohortSelectorError):
