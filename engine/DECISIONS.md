...
| D13 | AI default mode | **RESOLVED → default ON** (owner, 2026-06-27; supersedes §4 #4 / DIC-571 "default off/opt-in"). `map.js getAiMode` falls back to `'on'`; an explicit user choice (localStorage) still wins, and a deployment opts out via `COUNTY.ai.defaultMode='off'`. A **first-run pill** (`pv-ai-mode.js`, shown once via `pv-ai-pill-seen`, anchored under `#pv-ai-toggle`) points users to the toggle. AI-optional invariants intact — off still degrades to facts (facts-parity, B4 auto-fallback). Shipped `302e94e`. |
