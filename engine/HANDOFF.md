...
- **Repos (all `main`):** parcel-viewer `e56a925` (keystone Phases 1–3 landed this session; push pending) · ZIP/zip-poc `17ff90d` · county-data-services `0838fd5`. CI: `.github/workflows/isv-harness.yml`.
- **Tests:** 211 green (142 Node `node:test` + 69 Python `unittest`). Run: `bash engine/run-harness.sh`.
...
- **Phase 3 — DONE (Slice A `cb8cf67` + B1 `17f4938` + B2 `e56a925`):** (A) parcel source identity (`SOURCE_LAYER`/idField) from manifest, exposed `window.PV_PARCEL_SOURCE`. (B1) grew the manifest source shape (assembler carries `source`/`geomType`/`dbSource`/`fields`/`default`/`outlineOnly` + `style` + `role`). (B2) `pg-layers.js` overlay list + paint and `layer-registry.js` read from `PV_MANIFEST.vectorOverlays()/source(id)` (COUNTY fallback). Deferred: `county-layers.js` countyOverlays, WMS, and `_PARCEL_INFO_SOURCE` popup.sections. (DIC-407.)
- **Phase 4 (next):** finish the A3 read-migration grind. **Phase 5:** ZIP-as-a-theme (gated on ZIP runnable).
