...
- **Tests:** 127 green (109 Node `node:test` + 18 Python `unittest`). Run: `bash engine/run-harness.sh`.
...
| — | A6 backend convergence — **KnowledgeStore** (decouple knowledge retrieval; DICE + ZIP-local backends) | 570 | in progress (KnowledgeStore seam built & harness-verified; A6-a resolved. ParcelStore ZIP impl still blocked on Drake's parcel-data migration) |
...
4. **A6** — A6-a now resolved (option a). **KnowledgeStore done** (`ZIP/zip-poc/backend/knowledge_store.py`, jurisdiction-scoped DICE + ZIP-local backends; `tools.py` delegates; harness-tested). Next A6 steps: (a) flip ZIP to `ZIP_KNOWLEDGE_BACKEND=dice` once db-dice creds are wired + live-smoke the converged path; (b) `ParcelStore` interface + PV(VBC) impl (unblocked); (c) `ParcelStore` ZIP/Lockport impl waits on Drake's parcel-data migration + the `assessing.sjc_parcels`-vs-generalized table decision; (d) the larger map-buddy↔ZIP execution-layer merge.
