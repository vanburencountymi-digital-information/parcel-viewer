...
- **Repos (all `main`):** parcel-viewer `9ca843e` (keystone Phases 1–3 + Phase 3 depth landed this session; push pending) · ZIP/zip-poc `17ff90d` · county-data-services `0838fd5`. CI: `.github/workflows/isv-harness.yml`.
- **Tests:** 213 green (144 Node `node:test` + 69 Python `unittest`). Run: `bash engine/run-harness.sh`.
...
- **Phase 4 (next):** finish the A3 read-migration grind (file-by-file `window.PS_*` → context, parcel-studio re-verified per batch). **Phase 5:** ZIP-as-a-theme (gated on ZIP runnable).
